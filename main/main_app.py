...
Run: streamlit run main/main_app.py
"""
...
if page == "Overview":
    ...
    if total == 0:
        st.warning("No predictions in database yet. Run: `python pipeline/predict_batch.py --input data/gdelt_raw.csv --model baseline`")
        st.stop()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        HAS_MPL = False
        st.warning("pip install matplotlib")

    from nlp.attention_viz import get_attention_and_prediction

    headline_input = st.text_area(
        "Enter headline to analyze",
        placeholder="US imposes 145% tariffs on Chinese goods amid escalating trade war",
        height=80,
    )
    pair_input = st.selectbox("Bilateral pair", all_pairs,
    ...
    if st.button("Analyze Attention", type="primary") and headline_input.strip():
        ...
        <div style='border: 2px solid {color}; border-radius: 8px; padding: 12px; margin: 8px 0;'>
            <h3 style='color:{color}; margin:0'>{label.upper()} — {conf*100:.1f}% confidence</h3>
        </div>
        """, unsafe_allow_html=True)

        col1, col2, col3 = st.columns(3)
        for col, lab in zip([col1, col2, col3], ["adversarial", "cooperative", "neutral"]):
            col.metric(lab.capitalize(), f"{result['probs'][lab]*100:.1f}%")

        if HAS_MPL:
            # Build inline heatmap
            tokens = result["tokens"][:30]
            attn = result["attention"][:30]
            total_a = sum(attn) or 1.0
            attn = [a / total_a for a in attn]

            fig, ax = plt.subplots(figsize=(max(10, len(tokens) * 0.55), 2.2))
            fig.patch.set_facecolor("#0e1117")
            ax.set_facecolor("#0e1117")

            attn_arr = np.array(attn).reshape(1, -1)
            ax.imshow(attn_arr, cmap="YlOrRd", aspect="auto", vmin=0)
            ...
            ax.set_title("Token Attention Weights (CLS → tokens)", color="white", fontsize=10)
            plt.tight_layout()
        ...
        token_attn = sorted(zip(result["tokens"], result["attention"]),
                            key=lambda x: -x[1])[:10]
        for tok, a in token_attn:
            bar = "█" * int(a * 200)
            st.markdown(f"`{tok}` {bar} {a:.4f}")

    # Show pre-generated top words summary if it exists
    summary_img = ROOT / "results" / "attention" / "top_words_summary.png"
    if summary_img.exists():
        st.markdown("---")
        st.markdown("### Top Attention Words per Class (from test set)")
        st.caption("Generated by running: `python nlp/attention_viz.py`")
